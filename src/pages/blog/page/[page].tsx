...
const BlogPage = ({ strapiData }: { strapiData: StrapiPostWithMeta }) => {
...
export async function getStaticPaths() {
  let paths = [];
  const strapiData = await getAllPosts();
  for (let i = 0; i < strapiData.meta.pagination.pageCount; i++) {
...
export async function getStaticProps(context) {
  const strapiData = await getPostsByPage(context.params.page);
  return {
    props: {
      strapiData,
    },
